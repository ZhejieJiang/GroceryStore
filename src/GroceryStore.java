...
public class GroceryStore
{
    ...
    public ArrayList<Integer> getReorderList(int min)
    {
        ...
        {
            Product product = productsStocked[i];
            if(product.getQuantity() <= min){reorderList.add(i);}
        }
        return reorderList;
    }
    ...
    {
        ArrayList<Product> oversizedList = new ArrayList<Product>();
        for(int i = 0; i< productsStocked.length; i++)
        {
            Product product = productsStocked[i];
            if(product.getWeight() > weight){oversizedList.add(productsStocked[i]);}
        }
